...
public class MixAll {
    ...
    public static void properties2Object(final Properties p, final Object object) {
        Method[] methods = object.getClass().getMethods();//获取public类型的方法
        for (Method method : methods) {
            String mn = method.getName();//获取public类型的方法名称
            if (mn.startsWith("set")) {
                try {
                    ...
                    String property = p.getProperty(key);//获取set方法对象并从property中得到值
                    if (property != null) {
                        Class<?>[] pt = method.getParameterTypes();//方法的参数列表
